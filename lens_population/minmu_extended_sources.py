    ...
    if nsource > 0:
        ...
        while not islens and n < nsource:
            ...
            if detection:
                ...
                det_mu_list.append(det_mu)

                global_mu_list.append(global_mu)

                if not np.isnan(det_mu):
                    hdr['det_mu'] = det_mu
                else:
                    hdr['det_mu'] = 0.

                hdr['glob_mu'] = global_mu
                ...
                hdulist.writeto(modeldir+'/minmulens_%06d.fits'%i, overwrite=True)
...
if 'minmulens' in pop:
    ...
    pop.create_dataset('minmulens', data=islens_samp)

if 'minmutein_zs' in pop:
    data = pop['minmutein_zs']
    data[()] = tein_zs_samp

else:
    pop.create_dataset('minmutein_zs', data=tein_zs_samp)

if 'minmurein_zs' in pop:
    data = pop['minmurein_zs']
    data[()] = rein_zs_samp

else:
    pop.create_dataset('minmurein_zs', data=rein_zs_samp)

# makes file with lenses

lens_file = h5py.File('%s_minmulenses.hdf5'%modelname, 'w')

lens_file.attrs['minmu'] = minmu
