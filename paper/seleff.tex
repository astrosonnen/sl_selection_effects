...
The reason for this discrepancy lies in the differences bewteen the lens mass models in the two simulations.
...
Each one of these conditions introduces a bias with respect to the parent population of foreground galaxies and background sources. Together, they define the lens detection probability term $\pdet$ of \Eref{eq:one}.
...
The best case scenario occurs when the efficiency of including a detected strong lens in a survey is always one. In this case, the third condition does not introduce any further selection and the strong lensing bias is minimised.
...
In the fiducial scatter scenario, the one consistent with the observed scatter around the fundamental plane, the bias on $\asps$ varies from $0.03$~dex, when no restrictions on the lens Einstein radius are applied, to $0.08$~dex, corresponding to the case in which only lenses with $\tein > 2.0''$ are selected (see \Fref{fig:lensmassbias}).
The current systematic uncertainty on $\asps$ is $0.2-0.3$~dex: this is roughly the difference in mesaurements of the stellar mass of a galaxy obtained with a Chabrier or a Salpeter IMF.
...
For instance, at fixed stellar mass and half-light radius, the bias on the projected dark matter mass enclosed within $5$~kpc, $\mdmfive$, is only a few percent in the fiducial scatter scenario with $\theta_{\mathrm{Ein,min}} < 1.0''$.
...
SDSS spectra were taken in fibers with a $1.5''$ radius.
Lensed galaxies that are comparable in size to this scale, or larger, are less likely to be detected, because part of their flux extends outside of the fiber. Compact galaxies with a large overall magnification, instead, are more likely to be detected \citep[see the discussion in section 5.3 of][]{O+A17}.
...
On the other hand, these biases are a function of the minimum Einstein radius. In order to use a sample of strong lenses as a prior for another sample, then, it is important to make sure that 1) the parent population of foreground galaxies among which lenses are searched for is the same in both surveys, and 2) the two surveys can probe the same distribution in Einstein radius.
...
This is directly related to the projected ellipcitiy: galaxies with an axis ratio close to one tend to be preferentially elongated in the line-of-sight direction, and viceversa.
...
This result suggests that the dependence of the strong lensing bias on the deails of the background source population is very weak, as long as no additional selection on the image configuration is applied (e.g. by selecting only quad lenses).
...
Doing so, however, can be difficult in practice.
...
%An estimate of the bias, however, could be obtained after constraining the lens population distribution, by creating a simulation of the survey under the assumption that the inferred distribution is a good approximation of the truth. The end product would be a posterior prediction of the properties of a sample of strong lenses. This procedure could then be iterated until convergence.
%If the inferred intrinsic scatter at fixed observable properties is small, so will be the amplitude of the strong lensing bias.

%If one wishes to directly correct for the strong lensing bias but wants to avoid to explicitly model all of the aspects entering the selection process, one possibility is to try and approximate $\pdet$. 
If one wishes to directly correct for the strong lensing bias in a computationally manageable way, one possibility is to try and approximate $\pdet$. 
...
This requires in principle to explicitly model the distribution of all of these quantities \citep[as done by][]{Son22}.
...
%However, we can make the assumption that the dependence of $\pdet$ on source position and surface brightness distribution can be separable from the dependence on the lens mass properties and the lens and source redshift.
%This is a reasonable assumption, given that the dependence of the strong lensing cross-section on the source parameters is similar in lenses with different mass properties (see section \ref{ssec:axisymmpoint}).
%This assumption would allow us to marginalise over the source parameters, simplifying the problem. The lens detection probability would then be reduced to the sole $\crosssect$, computed 
