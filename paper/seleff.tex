...
%This assumption is probably broken in real galaxies but, as we discuss in \Sref{ssec:limitations}, does not impact our investigation.
...
We adopted a few different values for the scatter $\sigma_\alpha$. We explain in \ref{ssub:scat} how these were chosen.
...
For the simulation with extended sources, we produced images with properties similar to those expected for the Euclid Wide survey 
citep{Sca++22}.
...
A completeness limit of $0.5''$ is close to what can currently be achieved via visual inspection of high-resolution space-based images \cite{Gar++22}, while the $1.0''$ limit can be seen as a more conservative case.
\begin{figure*}
...
%Allowing for additional flexibility at fixed halo mass scatter parameter $\sigma_h$ results in a larger scatter in the inner dark matter distribution, which would increase the amplitude of the lensing bias. However, the value of $\sigma_h$ that we chose for our fiducial experiment was tuned on the basis of the predicted scatter in velocity dispersion around the fundamental plane. That scatter would also be increased by adding flexibility to the dark matter density profile. Then, in order to be consistent with the analysis carried out in our paper, the value of $\sigma_h$ would need to be lowered accordingly. This, in turn, would reduce the amplitude of the strong lensing bias.
Since lensing and dynamics are sensitive to the mass distribution at comparable scales (the half-light radius and Einstein radius are similar for most of the lenses), we expect these two effects to cancel out to first approximation. The lensing bias on quantities directly related to the inner mass distribution, such as $\asps$ and $\mdmfive$, would be left roughly unchanged. %leaving the strong lensing bias on roughly unchanged.
...
%After a thorough investigation of the strong lensing bias in photometrically selected samples of strong lenses, we learned the following important lessons. 
%After a thorough investigation of the strong lensing bias, we learned the following important lessons that apply to photometrically selected samples of strong lenses.
...
%The thorough investigation of the strong lensing bias that we carried out in this work allowed us to learn several important lessons.
\begin{enumerate}
\item The strong lensing cross-section increases primarily with increasing lens mass, with decreasing half-mass radius, and with increasing source brightness. However, it has little dependence on the size of the source, if this is smaller than the Einstein radius and if it is detectable in the absence of lensing. %source size when this is smaller than the Einstein radius. 
%Sources with a surface brightness that is too low to be detected in the absence of lensing are still undetected when strongly lensed. Similarly, sources that are detectable 
\item 
