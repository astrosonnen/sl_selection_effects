...
In the case of an extended source, the complexity of the problem is increased due to the addition of a series of features: the surface brightness distribution including its radial profile, shape and orientation, and the point spread function (PSF). 
For the sake of reducing the dimensionality of the problem, we neglect the PSF and focus on circularly symmetric sources. We also fix the surface brightness profile to an exponential disk (i.e. a S\'{e}rsic model with $n=1$).
...
We carried out experiments with sources with different values of the total flux and varying half-light radius, $\theta_s$.
The results are shown in \Fref{fig:extcs}.
The total flux of the source, indicated in the legend, is expressed in terms of the sky background fluctuation within an area equal to the square of the Einstein radius. We assumed that the background noise is an uncorrelated Gaussian field, for this purpose.
...
As in the point source case, the lensing cross-section increasing with increasing total flux, at fixed source size.
...
In order to determine whether there are lensing-specific features in the $\crosssect-\theta_s$ relation of \Fref{fig:extcs}, we computed, for each source flux, the maximum half-light radius for which it can be detected in the absence of lensing.
...
The resulting limiting sizes are shown as vertical lines in \Fref{fig:extcs}. For each given total flux, the non-lensing size limit is larger than the value of $\theta_s$ at which the lensing cross-section drops.
This implies that strong lensing selects preferentially more compact sources, compared to the field.
