...
labels = ['$\log{M_*^{(\mathrm{obs})}}$', '$\log{R_{\mathrm{e}}}$']

lims = [(11., 12.1), (0.4, 1.8)]

major_step = [0.5, 0.5]
minor_step = [0.1, 0.1]

fig = pylab.figure()
pylab.subplots_adjust(left=0.12, right=0.99, bottom=0.17, top=0.99, hspace=0.1, wspace=0.1)
...
for i in range(npars):
    ...
    pylab.hist(gal_samp[pars[i]], bins=bins, color=gal_color, histtype='stepfilled', weights=gweights, linewidth=3, label='General population')

    lweights = np.ones(nlens)/float(nlens)/(bins[1] - bins[0])
    pylab.hist(lens_samp[pars[i]], bins=bins, color=lens_color, histtype='step', weights=lweights, linewidth=3, label="Lenses, $\\theta_{\mathrm{Ein}} > 0.5''$")

    tweights = np.ones(ntein1)/float(ntein1)/(bins[1] - bins[0])
    pylab.hist(tein1_samp[pars[i]], bins=bins, color=tein1_color, histtype='step', weights=tweights, linewidth=3, linestyle='--', label="Lenses, $\\theta_{\mathrm{Ein}} > 1.0''$")
    ...
    if i==0:
        ...
        box = ax.get_position()
        ax.legend(loc='upper right', bbox_to_anchor=(2.1, 1.), fontsize=fsize, scatterpoints=3)
...
for j in range(1, npars): # loops over rows
    ...
    for i in range(j): # loops over columns
        ...
        probcontour(gal_samp[pars[i]], gal_samp[pars[j]], color=gal_color, style='filled')
        probcontour(lens_samp[pars[i]], lens_samp[pars[j]], color=lens_color, style='lines', linewidths=3, smooth=5)
        probcontour(tein1_samp[pars[i]], tein1_samp[pars[j]], color=tein1_color, style='lines', linewidths=3, linestyles='--', smooth=7)
...
pylab.savefig('../paper/mobsreff_cornerplot.png')
pylab.show()
