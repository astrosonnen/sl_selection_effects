...
for n in range(nsims):
    ...
    for i in range(ntein):
        ...
        lmst_here = lenspop['lmst'][lenscut].flatten()
        rein_here = lenspop['rein_zs'][lenscut]
        ...
        """
        # fits for the lambda_int vs. reff/rein relation

        def lmstfitfunc(p):
            return p[0] + p[1] * (10.**lreff_here/rein_here - 1.)

        def lmsterrfunc(p):
            return lmstfitfunc(p) - lmst_here

        pfit = leastsq(lmsterrfunc, [1., 0.1])

        lmst_0_here = pfit[0][0]
        lmst_scat = np.std(fitfunc(pfit[0]) - lmst_here)

        lmstmed_arr[i] = lmst_0_here
        lmsterr_arr[i] = lmst_scat/float(nlens)**0.5
        """

        # just takes the median: there's not much point in fitting a linear relation
        # to reff/rein, given that the distribution is not exactly linear
    ...
    ax[4].errorbar(tein_arr, lmstmed_arr, yerr=lmsterr_arr, color=colors[n], linewidth=2, label=labels[n], linestyle=linestyle)
...
#ax[4].set_ylabel('$\lambda_{\mathrm{int},0}$ (Mean $\lambda_{\mathrm{int}}$ \n at $\\theta_{\mathrm{Ein}} = \\theta_{\mathrm{e}}$)', fontsize=fsize)
...
ax[2].legend(loc=(0.02, 0.5), fontsize=fsize, framealpha=1.)
