...
This follows from the fact that gravitational lensing preserves surface brightness: a source that can be detected in the absence of lensing will produce images with the same surface brightness when lensed, which can be detected as well.
...
In practice, as we explain in section \ref{ssec:limitations}, our main resultsare not affected by this choice.
...
We also assumed that the dark matter density profile of the galaxies is determined uniquely by the halo mass and by the stellar mass distribution (see section \ref{ssub:dmprofile}). In reality we expect there to be a range of density profiles, for example as a result of halos having a nonzero scatter in their initial concentration parameter.
...
Since lensing and dynamics are sensitive to the mass distribution at comparable scales (the half-light radius and Einstein radius are similar for most of the lenses), we expect these two effects to cancel out to first approximation, leaving the strong lensing bias roughly unchanged.
