...
\Fref{fig:lenspars} shows the distribution in the parameters of the foreground galaxies of the lens systems, compared to those of the general population, for the fiducial scatter scenario with two different values of the minimum Einstein radius: $0.5''$ and $1.0''$.
...
\caption{
Comparison bewteen the properties of lens samples and the general galaxy population. Distribution in foreground galaxy parameters.
\label{fig:lenspars}
}
\end{figure*}
...
The unlensed distribution (blue filled contours) was obtained by taking into account the detectability of the sources: we drew a large number of sources from the distribution described in section \ref{ssec:sources}, simulated images of them with the same properties as the images of the lenses, and selected only those galaxies detected with $S/N>10$.
Therefore, the resulting distribution is different to the parent distribution from which the simulation was drawn.

Since we want to quantify the effect of lensing with respect to the case with no lensing for a given survey, we took into account the detectability of the sources when determining the unlensed background galaxy distribution.
...
The distribution in source magnitude is also significantly affected by the strong lensing selection.
In a somewhat counter-intuitive way, the $\msource$ distribution of the lenses peaks at brighter magnitudes than the parent distribution. The parent sample, however, is simply the input of the simulations, and does not take into account whether sources are detectable or not. The detection limit that we assumed in our simulations is at $\msource\approx25$, and therefore the source magnitude distribution in the absence of lensing would drop rapidly. 
In the absence of lensing, this distribution would drop rapidly beyond the detection limit $\msource\approx25$. Lensing, however, allows the detection of sources at fainter magnitudes, in virtue of the magnification, and therefore the $\msource$ distribution shows a faint-end tail beyond $\msource=25$.
...
Qualitatively, the result shown in \Fref{fig:lenspars} matches our expectations from \Sref{sect:indlenses}.
In order to quantitatively assess the amplitude of the strong lensing bias in various lensing properties, we proceeded as follows.

\Fref{fig:sourcepars} shows the distribution in source properties.
