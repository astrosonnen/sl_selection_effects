...
\begin{equation}
\theta_{\mathrm{min}} < \theta_{\mathrm{s}} \leq \pi,
\end{equation}
with $\theta_{\mathrm{min}}=\pi/2$.
\Fref{fig:lensdef} shows a few examples of how this criterion can be used to classify lenses.


If no counter-image is detected, however, it can be difficult to distinguish such 

For instance, it can be difficult to distinguish a single arc with no detected counter-image from a merging set of three images
...
The reference lens is a galaxy at $z=0.3$ with $\log{\mstar}=11.5$, $\reff=7\,{\rm kpc}$, $\gammadm=1.5$, $\log{\mdmfive}=11.0$, and a source at $z=1.5$
